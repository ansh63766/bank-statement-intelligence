...
logger = logging.getLogger("bank_statement_intel.axis_parser")
...
class AxisParser(BaseParser):
    ...
    async def parse(self, pdf_path: str) -> ParsedStatement:
        metadata = StatementMetadata(bank_name="Axis")
        ...
        try:
                ...
                for page_idx, page in enumerate(pdf.pages):
                    ...
                    for table in tables:
                        for row in table:
                            ...
                            # Axis headers contain Date, Transaction Remarks / Particulars, Cheque No, Debit / Withdrawal, Credit / Deposit, Balance
                            is_header = any("date" in cell.lower() for cell in row_str) and \
                                        any("remarks" in cell.lower() or "narration" in cell.lower() or "particulars" in cell.lower() for cell in row_str) and \
                                        any("debit" in cell.lower() or "credit" in cell.lower() or "withdrawal" in cell.lower() or "deposit" in cell.lower() for cell in row_str)
            ...
            logger.error(f"Error parsing Axis Statement: {e}")
            raise e
    ...
    def _extract_metadata(self, text: str, metadata: StatementMetadata):
        """Extract Axis account details from text."""
        # Account Holder
        name_match = re.search(r"Name\s*:\s*(.*)", text, re.IGNORECASE)
        if name_match:
            metadata.account_holder = name_match.group(1).split("\n")[0].strip()
        else:
            name_match_alt = re.search(r"Customer Name\s*:\s*(.*)", text, re.IGNORECASE)
            if name_match_alt:
        ...
        if acc_match:
            ...
            acc_match_alt = re.search(r"Account Number\s*:\s*([A-Za-z0-9\-]+)", text, re.IGNORECASE)
            if acc_match_alt:
